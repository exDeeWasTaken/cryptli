...
deno_crypli_js=$(<$path_to_deno_crytli_js);
edited=${deno_crypli_js/"'cryptli_bg.wasm'"/"filePath"}
edited=${edited/"const file = new URL(import.meta.url).pathname;"/"const file = new URL(import.meta.url).pathname;
const filePath = Deno.build.os === 'windows' ? 'wasm/cryptli_bg.wasm' : 'cryptli_bg.wasm'"}
echo "$edited" > $path_to_deno_crytli_js